...
__all__ = (
    ...
    'HydrothermalLiquefactionMCA', 'HydrothermalLiquefactionKinetics'
    )
...
class HydrothermalLiquefactionMCA(Reactor):
    '''
...
# separator
@cost(basis='Treatment capacity', ID='Solids filter oil/water separator', 
      units='lb/h', cost=3945523, S=1219765, CE=CEPCI_by_year[2011], n=0.68, 
      BM=1.9)
class HydrothermalLiquefactionKinetics(Reactor):
    '''
    HTL Kinetics converts dewatered sludge to biocrude, aqueous, off-gas, and 
    hydrochar under varying elevated temperatures (250, 300, or 350°C), 
    pressure, and time. 
    
    The products percentage (wt%) can be evaluated using a first-order bulk
    kinetic model (Obeid et al., 2020) with known sludge composition (protein%,
    lipid%, lignin%, and carbohydrate%, all afdw%) and kinetic parameters for
    sludge (Obeid et al., 2022) and biosolids (Al-Jaboori et al., 2023)
...
#TODO add temp and time to HTL, add it as a property (adding property allows calling)
 ...
 #TODO add 'Sludge_lignin' to systems.py  
        ...
        self.afdw_protein_ratio = self.WWTP.sludge_afdw_protein
        self.afdw_carbo_ratio = self.WWTP.sludge_afdw_carbo
        self.afdw_lignin_ratio = self.WWTP.sludge_afdw_lignin
        ...
        def kinetics_odes(x, t):
            k1,k2,k3,k4,k5,k6,k7,k8,k9,k10,k11,k12,k13,k14,k15,k16,k17 =\
                kinetics_df[str(self.rxn_temp)]/60 # 60 converts from seconds to minutes
...
#TODO ask Jianan why gas_mass is used instead of gas.imass['Gas']
        #gas.imass['Gas']= gas_perc[rxn_time]*dewatered_sludge_afdw
        gas_mass = self.gas_perc[self.rxn_time]*\
                                        self.dewatered_sludge_afdw
